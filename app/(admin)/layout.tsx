...
export default function AdminLayout({
  ...
  const navItems = [
    ...
    { label: "Customer Orders", href: "/admin/orders", icon: History },
    ...
    <div className="flex flex-col md:flex-row min-h-screen bg-slate-950 text-slate-100">
      ...
      <aside className="w-64 border-r border-slate-900 bg-slate-950 p-6 hidden md:flex flex-col justify-between h-screen sticky top-0">
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`flex items-center gap-3 py-2.5 px-4 rounded-xl text-xs font-semibold transition-all ${isActive ? "bg-indigo-600 text-white shadow-lg shadow-indigo-500/20" : "text-slate-400 hover:text-slate-200 hover:bg-slate-900/60"}`}
                >
                  <Icon size={16} />
                  {item.label}
                </Link>
        ...
        <div className="flex flex-col gap-2 border-t border-slate-900 pt-6">
          ...
          <Link
            href="/"
            className="flex items-center gap-3 py-2 px-3 rounded-lg text-xs text-slate-400 hover:text-indigo-400 hover:bg-slate-900/40 transition-colors"
          >
            <Store size={14} />
            Go to Storefront
          </Link>
                ...
                {navItems.map((item) => {
                  ...
                  return (
                    <Link
                      ...
                      className={`flex items-center gap-3 py-2.5 px-4 rounded-xl text-xs font-semibold transition-all ${isActive ? "bg-indigo-600 text-white" : "text-slate-400 hover:text-slate-200"}`}
                    >
                      <Icon size={16} />
                      {item.label}
                    </Link>
